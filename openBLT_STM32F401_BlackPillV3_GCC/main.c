...
int main(void) {
  ...
  /* the bootloader is configured to NOT initialize the TCP/IP network stack by
   * default to bypass unnecessary delay times before starting the user program.
   * the TCP/IP net- work tack is now only initialized when: (a) no valid user
   * program is detected, or (b) a forced backdoor entry occurred
   * (CpuUserProgramStartHook() returned BLT_FALSE).
   *
   * these demo bootloader and user programs have one extra feature implemented
   * for demonstration purposes. the demo user program can detect firmware
   * update requests from the TCP/IP network in which case it activates the
   * bootloader. But...the TCP/IP network stack will not be initialized in this
   * situation. for this reason the shared parameter module was integrated in
   * both the bootloader and user program. more information about the shared
   * parameter module can be found here: https://www.feaser.com/en/blog/?p=216
   *
   * the shared parameter at the first index (0) contains a flag. this flag is
   * set to 1, right before the user program activates this bootloader, to
   * explicitly request the bootloader to initialize the TCP/IP network stack.
   * this makes it possible for a firmware update to proceed. the code here
   * reads out this flag and performs the TCP/IP network stack initialization
   * when requested.
   */
